    ...
    while(!stop_condition){
        ...
        switch (stoi(pack[0])) {
            case MSG:
                ...
                sendto(sockfd, dp.pack(TYPE_ACK,stoi(pack[1]),"ACK_MESSAGE").c_str(), strlen(dp.pack(TYPE_ACK,stoi(pack[1]),"ACK_MESSAGE").c_str()), 0, (const struct sockaddr *)&server_addr, addr_len);
